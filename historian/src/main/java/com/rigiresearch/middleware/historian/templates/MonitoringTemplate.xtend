...
final class MonitoringTemplate {
    ...
    /**
        ...
        def asProperties(Root root) '''
            ...
            # If the API calls require authentication, specify a path id from the previous
            # list to request an authentication token:
            #
            # auth=<path-id>
            #
            # Now, setup basic HTTP authentication for the auth path. In this case, the
            # values are taken from the environment variables API_AUTH_USERNAME and
            # API_AUTH_PASSWORD.
            #
            # <path-id>.username=${env:API_AUTH_USERNAME}
            # <path-id>.password=${env:API_AUTH_PASSWORD}
            #
            ...
            #
            # <path-id>.output.parameters=token
            # <path-id>.output.parameters.token.selector=auth_token
            #
            # You may use the value "${<path-id>.output.parameters.token.value}" as a
            # parameter value for other paths. For example:
            #
            # myPathId.parameters.param1.value=${myAuth.output.parameters.token.value}
            #
            ...
            # In the case of the auth path, choose an appropriate expression to request an
            ...
            #
            # myPathId.parameters.param1.value=\
            # {\
            #   "auth": {\
            #     "methods": ["password"]\,\
            #     "password": {\
