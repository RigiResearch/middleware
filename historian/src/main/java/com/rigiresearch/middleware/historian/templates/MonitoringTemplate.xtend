...
final class MonitoringTemplate {
    ...
    def populateDefaultProperties(PropertiesConfiguration config, Root root) {
        ...
        config.setProperty("monitors", root.monitors.map[m|m.path.id].join(", "))
        config.layout.setComment("monitors", "The following are paths from the API specification")

        config.setProperty("thread.pool.size", "30")
        config.layout.setBlancLinesBefore("thread.pool.size", 1)
        config.layout.setComment("thread.pool.size", "The pool size for concurrent requests")

        config.setProperty("DEFAULT_DELAY_EXPR", "* * * * *")
        config.layout.setBlancLinesBefore("DEFAULT_DELAY_EXPR", 1)
        config.layout.setComment("DEFAULT_DELAY_EXPR", "Cron expression requesting data every minute")
        ...
        for (m : root.monitors) {
            ...
            config.setProperty(
                '''«m.path.id».response.class''',
                '''«MonitoringTemplate.PACKAGE».«m.path.id.asClassName»'''.toString
            )
            config.setProperty(
                '''«m.path.id».expression''',
                '''«IF m.rate.value === null»${DEFAULT_DELAY_EXPR}«ELSE»«m.rate.value»«ENDIF»'''.toString
            )
        }
        return config
    }
    ...
    def populateCustomProperties(PropertiesConfiguration config, Root root) {
        config.layout.globalSeparator = "="
        val header = <String>newArrayList
        header.add('''
        # File generated by Historian («new Date()»)
        #
        # AUTHENTICATION
